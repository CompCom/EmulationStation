...
public:
	...
	void setText(const std::string& text, const std::string& helpText, bool upperCase, bool resize = true, bool doUpdateHelpPrompts = true);
	void autoSizeFont();
	...
	inline void setColorShift(unsigned int color) { mModdedColor = color; mNewColor = true; updateImage(); }
	inline void removeColorShift() { mNewColor = false; updateImage(); }
